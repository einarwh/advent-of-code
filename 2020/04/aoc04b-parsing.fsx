...
module Domain =
    ...
    module Passport =
        ...
        let parse (s : string) : Passport option =
            ...
            let maybeBirthYear = lookup "byr" |> Option.bind (BirthYear.parse)
            let maybeIssueYear = lookup "iyr" |> Option.bind (IssueYear.parse)
            let maybeExpirationYear = lookup "eyr" |> Option.bind (ExpirationYear.parse)
            let maybeHeight = lookup "hgt" |> Option.bind (Height.parse)
            let maybeHairColor = lookup "hcl" |> Option.bind (HairColor.parse)
            let maybeEyeColor = lookup "ecl" |> Option.bind (EyeColor.parse)
            let maybePassportId = lookup "pid" |> Option.bind (PassportId.parse)
            match (maybeBirthYear, maybeIssueYear, maybeExpirationYear, maybeHeight, maybeHairColor, maybeEyeColor, maybePassportId) with
