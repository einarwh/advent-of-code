...
calc : Int -> Float 
calc n = 300.0 + (toFloat n / 6)

toPolylineElement : List Position -> Svg msg 
toPolylineElement positions = 
  let 
    s = 
      let str (xval, yval) = (String.fromFloat (calc xval)) ++ "," ++ (String.fromFloat (calc yval))
      in
...
toShipElement (xval, yval) = 
  let 
    ...
    yStr = String.fromFloat (calc yval - 35.00)
  in 
...
view model =
  let
    ...
    dist = distance model.pos
    distanceStr = String.fromInt dist
    shipElements = [ toShipElement model.pos ]
