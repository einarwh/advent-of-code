...
updateStep model = 
  ...
  in 
    case model.moves of 
      [] -> 
        { model | position = position, paused = True, finished = True, debug = "paused?" }
      move :: rest -> 
        case move of 
          Left steps -> 
            if steps > 0 then 
              ...
              let 
                zeroes = model.zeroes + if position == 0 then 1 else 0
              in 
                { model | moves = rest, position = position, zeroes = zeroes, debug = "L done" }
          Right steps -> 
            if steps > 0 then 
              let 
              ...
              let 
                zeroes = model.zeroes + if position == 0 then 1 else 0
              in 
                { model | moves = rest, position = position, zeroes = zeroes, debug = "R done" }
...
toNumberElement angle = 
  let 
    ...
    x1Str = 160 * xPos |> String.fromFloat 
    x2Str = 166 * xPos |> String.fromFloat
    y1Str = 160 * yPos |> String.fromFloat 
    y2Str = 166 * yPos |> String.fromFloat
    lineElement = line [ x1 x1Str, x2 x2Str, y1 y1Str, y2 y2Str, stroke "currentcolor" ] []
  in 
    [ lineElement ]

toDialElement : Int -> List (Html Msg)
toDialElement number = 
  let 
    angle = 3.6 * toFloat number 
    xPos = cos (degrees (90 - angle))
    yPos = -1 * sin (degrees (90 - angle))
    x1Str = 100 * xPos |> String.fromFloat 
    x2Str = 150 * xPos |> String.fromFloat
    y1Str = 0 * yPos |> String.fromFloat 
    y2Str = 150 * yPos |> String.fromFloat
    ...
    Html.table 
      ...
      [ Html.tr 
          ...
          [ Html.td 
              ...
              , Html.div [] [Html.text "Day 1: Secret Entrance" ] ] ]
      , Html.tr 
          []
          [ Html.td 
              [ Html.Attributes.align "center"
              ...
              [ Html.a 
                [ Html.Attributes.href "https://adventofcode.com/2025/day/1" ] 
                [ Html.text "https://adventofcode.com/2025/day/1" ]
            ] ]
      , Html.tr 
          []
          [ Html.td 
