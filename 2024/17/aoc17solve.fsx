...
let findQuine computer = 
    printfn "\nfindQuine"
    let len = computer.program |> Array.length
    // let len64 = int64 len 
    let rec pow x y = if y < 1L then 1L else x * pow x (y - 1L)
    let rec findNext target (a : int64) (offset : int64) i j = 
        printfn "findNext with target=%d, a=%d, offset=%d, i=%d, j=%d" target a offset i j 
        if j > 8 then failwith "?"
        else 
            let nextA = a + (int64 j) * offset
            let p = execute { computer with regA = nextA }
            let n = p.[len - i]
            printfn "Number: %d" n
            if n = target then 
                nextA 
            else 
                findNext target a offset i (j + 1)
    ...
    let rec loop (a : int64) (ix : int) : int64 option = 
        // printfn ""
        // printfn "Loop with a=%A, index=%d" a ix 
        let opIndex = len - ix 
        if ix > len then 
            Some a
        else 
