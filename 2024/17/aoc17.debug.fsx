...
let printComputer step computer = 
    printfn "\nComputer at step %d" step
    printfn "---------------"
    printfn "A: %d" computer.regA
    printfn "B: %d" computer.regB
    printfn "C: %d" computer.regC
    // printfn "I: %d" computer.pointer
    printfn "P: %s" (computer.program |> Array.map string |> String.concat ",")
    let ptr = "^".PadLeft(2 * (1 + (int) computer.pointer))
    printfn "  %s" ptr
    printfn "O: %s" (computer.outputs |> List.rev |> List.map string |> String.concat ",")

let rec execute step computer =
    match tryReadOpcode computer with
    | None ->
        computer.outputs |> List.rev |> List.toArray 
    | Some opcode ->
        printComputer step computer 
        let c =
            match opcode with
            | 0 -> adv computer
            | 1 -> bxl computer
...
let quine computer = 
    ...
    let checkTarget opIndex target candidateA = 
        let p = execute 0 { computer with regA = candidateA }
        p.[opIndex] = target 
    let rec loop ix a = 
        let opIndex = len - ix 
        if ix > len then 
...
let run fileName =
    ...
    let a = 108107566389757L
    let c = { computer with regA = a }
    c |> execute 0 |> printfn "%A"
