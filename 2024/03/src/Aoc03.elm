...
moveAmount : Int -> Int -> Int -> Stacks -> Stacks
moveAmount amount source target stacks = 
    case takeAmount amount [] (Dict.get source stacks |> Maybe.withDefault []) of 
        (taken, stack) -> 
            stacks 
            |> Dict.update source (Maybe.map (\old -> stack))
            |> Dict.update target (Maybe.map (\old -> taken ++ old))

runCommand : Command -> Stacks -> Stacks 
runCommand command stacks = 
    moveAmount command.amount command.source command.target stacks
