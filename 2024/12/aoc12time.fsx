...
let rec fill garden plant (x, y) plot = 
    if Set.contains (x, y) plot then plot 
    else 
        match Garden.tryGet garden (x, y) with 
        | Some p when p = plant -> 
            plot 
            |> Set.add (x, y)
...
let rec fillLoop garden plant candidates plot =
    let inside = candidates |> List.filter (Garden.inBounds garden) 
    let unseen = inside |> List.filter (fun pos -> not <| Set.contains pos plot) 
    let verified = unseen |> List.filter (fun pos -> plant = Garden.get garden pos) 
    let nextCandidates = List.collect neighbours verified |> Set.ofList |> Set.toList 
    if List.length nextCandidates = 0 then 
        plot 
    else 
        let nextPlot = Set.union plot (Set.ofList verified)
        fillLoop garden plant nextCandidates nextPlot

let fillPlot garden (x, y)  = 
    let plant = Garden.get garden (x, y)
    // fill garden plant (x, y) Set.empty
    fillLoop garden plant [(x, y)] Set.empty
