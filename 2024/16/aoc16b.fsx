...
let solve1 startPos (maze : Maze) = 
    let rec loop (visited : Set<Path>, q : PQ, combined : Set<Pos>) = 
        // printfn "q elements: %d" q.Count 
        if q.Count = 0 then combined |> Set.count
        else 
            ...
            | 'E' -> 
                // End!
                // printfn "%d" distance
                let positions = visited |> Set.map (fun path -> path.pos) 
                let combined' = Set.union combined positions 
                loop (nextVisited, q, combined') 
                // Some distance 
            | 'S' 
            | '.' ->
                // Free space!
    ...
    let result = loop (Set.empty, queue, Set.empty)
    printfn "%A" result
    0 
...
let run fileName =
    ...
    solve1 startPos maze |> printfn "%A"
