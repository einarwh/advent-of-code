...
var $author$project$Main$checkCorners = F2(
	function (cornersStuckOn, grid) {
		if (cornersStuckOn) {
			var yMax = $tortus$elm_array_2d$Array2D$columns(grid) - 1;
			var xMax = $tortus$elm_array_2d$Array2D$rows(grid) - 1;
			return A4(
				...
				A4(
					...
					A4(
						...
						0,
						true,
						A4($tortus$elm_array_2d$Array2D$set, 0, 0, true, grid))));
		} else {
			return grid;
		}
	});
...
var $author$project$Main$initGrid = function (dataSource) {
	var data = function () {
		if (dataSource === 1) {
...
var $author$project$Main$initModel = F2(
	function (dataSource, cornersStuckOn) {
		var grid = A2(
			$author$project$Main$checkCorners,
			cornersStuckOn,
			$author$project$Main$initGrid(dataSource));
		return {q: cornersStuckOn, U: 0, t: dataSource, W: '', F: grid, L: '', w: true, y: 0, z: $author$project$Main$defaultTickInterval};
	});
...
var $author$project$Main$subscriptions = function (model) {
	var tickSub = model.w ? $elm$core$Platform$Sub$none : A2(
		$elm$time$Time$every,
		model.z,
		function (_v0) {
			return 0;
...
var $author$project$Main$updateClear = function (model) {
	return A2($author$project$Main$initModel, model.t, model.q);
};
var $author$project$Main$getNestedPositions = function (grid) {
	var ys = A2(
		$elm$core$List$range,
		0,
		$tortus$elm_array_2d$Array2D$rows(grid) - 1);
	var xs = A2(
		$elm$core$List$range,
		0,
		$tortus$elm_array_2d$Array2D$columns(grid) - 1);
	return A2(
		$elm$core$List$map,
		function (y) {
			return A2(
...
var $author$project$Main$getNeighbours = F2(
	function (grid, _v0) {
		var x = _v0.a;
		var y = _v0.b;
		...
		return A2(
			...
			function (_v1) {
				var xx = _v1.a;
				var yy = _v1.b;
				return A3($tortus$elm_array_2d$Array2D$get, yy, xx, grid);
			},
			positions);
	});
var $author$project$Main$isCorner = F2(
	function (grid, _v0) {
		var x = _v0.a;
		var y = _v0.b;
		var yMax = $tortus$elm_array_2d$Array2D$columns(grid) - 1;
		var xMax = $tortus$elm_array_2d$Array2D$rows(grid) - 1;
		return ((!x) && (!y)) || ((_Utils_eq(x, xMax) && (!y)) || (((!x) && _Utils_eq(y, yMax)) || (_Utils_eq(x, xMax) && _Utils_eq(y, yMax))));
	});
var $author$project$Main$isLightOn = F2(
	function (grid, _v0) {
		var x = _v0.a;
		var y = _v0.b;
		return A2(
			$elm$core$Maybe$withDefault,
			false,
			A3($tortus$elm_array_2d$Array2D$get, y, x, grid));
	});
var $elm$core$Basics$not = _Basics_not;
var $author$project$Main$turnOn = F3(
	function (cornersStuckOn, grid, pos) {
		if (cornersStuckOn && A2($author$project$Main$isCorner, grid, pos)) {
			return true;
		} else {
			var nbCount = $elm$core$List$length(
				A2(
					$elm$core$List$filter,
					$elm$core$Basics$identity,
					A2($author$project$Main$getNeighbours, grid, pos)));
			var currentlyOn = A2($author$project$Main$isLightOn, grid, pos);
			return (currentlyOn && ((nbCount === 2) || (nbCount === 3))) || ((!currentlyOn) && (nbCount === 3));
		}
	});
var $author$project$Main$step = F2(
	function (cornersStuckOn, grid) {
		var nestedPositions = $author$project$Main$getNestedPositions(grid);
		var g = $tortus$elm_array_2d$Array2D$fromList(
			A2(
				$elm$core$List$map,
				function (positions) {
					return A2(
						$elm$core$List$map,
						A2($author$project$Main$turnOn, cornersStuckOn, grid),
						positions);
				},
				nestedPositions));
		return g;
...
var $author$project$Main$updateStep = function (model) {
	var pause = (model.y + 1) === 100;
	var g = A2($author$project$Main$step, model.q, model.F);
	return _Utils_update(
		model,
		{F: g, w: pause, y: model.y + 1});
};
var $author$project$Main$updateToggleCornersStuckOn = function (model) {
	var cornersStuckOn = !model.q;
	return A2($author$project$Main$initModel, model.t, cornersStuckOn);
};
...
var $author$project$Main$updateUseInput = function (model) {
	return A2($author$project$Main$initModel, 0, model.q);
};
var $author$project$Main$Sample = 1;
var $author$project$Main$updateUseSample = function (model) {
	return A2($author$project$Main$initModel, 1, model.q);
};
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg) {
			...
			case 3:
				return _Utils_Tuple2(
					$author$project$Main$updateToggleCornersStuckOn(model),
					$elm$core$Platform$Cmd$none);
			case 7:
				return _Utils_Tuple2(
...
var $author$project$Main$ToggleCornersStuckOn = 3;
...
var $author$project$Main$getAllPositions = function (grid) {
	var ys = A2(
		$elm$core$List$range,
		0,
		$tortus$elm_array_2d$Array2D$rows(grid) - 1);
...
var $author$project$Main$toStyledCharElement = F2(
	function (grid, _v0) {
		...
		var y = _v0.b;
		var enabled = A2(
			$author$project$Main$isLightOn,
			grid,
			_Utils_Tuple2(x, y));
		var symbol = enabled ? '#' : '.';
		var cssClass = function () {
			if (symbol === '#') {
				return 'draw-yellow adaptive';
			} else {
...
var $author$project$Main$view = function (model) {
	...
	var grid = model.F;
	var lightCount = $elm$core$List$length(
		A2(
			$elm$core$List$filter,
			$author$project$Main$isLightOn(grid),
			$author$project$Main$getAllPositions(grid)));
	var nestedPositions = $author$project$Main$getNestedPositions(grid);
	var nestedElements = A2(
		...
		function (positions) {
			return A2(
				$elm$core$List$map,
				$author$project$Main$toStyledCharElement(grid),
				positions);
		},
		nestedPositions);
	var elements = A3(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text('Day 18: Like a GIF For Your Yard')
									]))
							]))
					])),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$href('https://adventofcode.com/2015/day/18')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text('https://adventofcode.com/2015/day/18')
									]))
							]))
					])),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										model.w ? ((!model.y) ? $elm$html$Html$text('Play') : $elm$html$Html$text('Resume')) : $elm$html$Html$text('Pause')
									])),
								A2(
								$elm$html$Html$button,
								_List_fromArray(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(' Corners stuck on')
									]))
							]))
					])),
				A2(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										'Steps: ' + $elm$core$String$fromInt(model.y))
									])),
								A2(
								$elm$html$Html$div,
								_List_Nil,
								_List_fromArray(
									[
										$elm$html$Html$text(
										'Lights: ' + $elm$core$String$fromInt(lightCount))
									])),
								A2(
								$elm$html$Html$div,
								_List_Nil,
