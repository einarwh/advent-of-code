...
toChamberRow : Array2D Char -> List Int -> Int -> String 
toChamberRow chamber xs y = 
  let 
    s = xs |> List.filterMap (\x -> Array2D.get y x chamber) |> String.fromList
  in 
    "|" ++ s ++ "|"

toChamberRows : Int -> Array2D Char -> List String 
toChamberRows highest chamber = 
  let
    ys = List.range 0 (highest  + 3)
    xs = List.range 0 (Array2D.columns chamber - 1)
    rows = ys |> List.map (toChamberRow chamber xs)
  in 
    List.reverse ("+-------+" :: rows)
...
viewBody model =
  let
    ...
    warehouse = model.warehouse
    rows = toChamberRows model.highestRock model.chamber
    -- Insert robot symbol.
