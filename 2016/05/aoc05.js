...
var $author$project$Main$input = 'uqwqemis';
...
var $author$project$Main$initModel = function (improvedMechanism) {
	var blank = A2($elm$core$Array$repeat, 8, $elm$core$Maybe$Nothing);
	return {Y: '', N: $author$project$Main$input, D: false, n: blank, v: improvedMechanism, f: 0, ae: '', m: blank, r: true, U: $author$project$Main$defaultTickInterval};
};
...
var $author$project$Main$subscriptions = function (model) {
	var tickSub = (model.r || model.D) ? $elm$core$Platform$Sub$none : A2(
		$elm$time$Time$every,
		model.U,
		function (_v0) {
			return 0;
...
var $author$project$Main$findPos = F2(
	function (i, maybeChars) {
		findPos:
		while (true) {
			if (!maybeChars.b) {
				return i;
				...
				var h = maybeChars.a;
				var rest = maybeChars.b;
				if (h.$ === 1) {
					return i;
				} else {
					var $temp$i = i + 1,
						$temp$maybeChars = rest;
					i = $temp$i;
					maybeChars = $temp$maybeChars;
					continue findPos;
				}
			}
		}
...
var $author$project$Main$hack1Loop = F3(
	function (pos, index, model) {
		hack1Loop:
		while (true) {
			var doorCode = _Utils_ap(
				model.N,
				$elm$core$String$fromInt(index));
			var hash = $truqu$elm_md5$MD5$hex(doorCode);
			if (A2($elm$core$String$startsWith, '00000', hash)) {
				var hashChars = $elm$core$Array$fromList(
					$elm$core$String$toList(hash));
				var ch = A2(
					$elm$core$Maybe$withDefault,
					'_',
					A2($elm$core$Array$get, 5, hashChars));
				var p = A3(
					$elm$core$Array$set,
					pos,
					$elm$core$Maybe$Just(ch),
					model.m);
				return _Utils_update(
					model,
					{f: index + 1, m: p});
			} else {
				if (_Utils_cmp(index, model.f + 10000) < 0) {
					var $temp$pos = pos,
						$temp$index = index + 1,
						$temp$model = model;
					pos = $temp$pos;
...
var $author$project$Main$hack1 = function (model) {
	var pos = A2(
		$author$project$Main$findPos,
		0,
		$elm$core$Array$toList(model.m));
	return A3($author$project$Main$hack1Loop, pos, model.f, model);
};
var $elm$core$Basics$negate = function (n) {
	return -n;
};
var $author$project$Main$hack2Loop = F2(
		...
		while (true) {
			...
			if (A2($elm$core$String$startsWith, '00000', hash)) {
				...
				var ch = A2(
					$elm$core$Maybe$withDefault,
					'_',
					A2($elm$core$Array$get, 6, hashChars));
				if ((pos >= 0) && (pos < 8)) {
					var _v0 = A2(
						$elm$core$Maybe$withDefault,
						$elm$core$Maybe$Nothing,
						A2($elm$core$Array$get, pos, model.m));
					if (!_v0.$) {
						return _Utils_update(
							model,
							{f: index + 1});
				...
				var ch = A2(
					$elm$core$Maybe$withDefault,
					'_',
					A2($elm$core$Array$get, pos, hashChars));
				var g = A3(
					$elm$core$Array$set,
					pos,
					$elm$core$Maybe$Just(ch),
...
var $author$project$Main$hack2 = function (model) {
	return A2($author$project$Main$hack2Loop, model.f, model);
};
var $author$project$Main$updateStep = function (model) {
	var m = model.v ? $author$project$Main$hack2(model) : $author$project$Main$hack1(model);
	var count = $elm$core$List$length(
		A2(
			$elm$core$List$filterMap,
			$elm$core$Basics$identity,
			$elm$core$Array$toList(m.m)));
	var finished = count === 8;
	return _Utils_update(
		m,
		{D: finished, r: finished});
};
var $elm$core$Basics$not = _Basics_not;
var $author$project$Main$updateToggleImprovedMechanism = function (model) {
	var improvedMechanism = !model.v;
	return $author$project$Main$initModel(improvedMechanism);
};
var $author$project$Main$updateTogglePlay = function (model) {
	if (model.D) {
		var m = $author$project$Main$initModel(model.v);
		return _Utils_update(
			m,
			{r: false});
	} else {
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg) {
			...
			default:
				return _Utils_Tuple2(
					$author$project$Main$updateToggleImprovedMechanism(model),
					$elm$core$Platform$Cmd$none);
		}
	});
var $author$project$Main$Reset = 3;
var $author$project$Main$ToggleImprovedMechanism = 2;
var $author$project$Main$TogglePlay = 1;
...
var $author$project$Main$getChar = F3(
	function (p, g, index) {
		var pChar = A2(
			$elm$core$Maybe$withDefault,
			$elm$core$Maybe$Nothing,
			A2($elm$core$Array$get, index, p));
		var gChar = A2(
			$elm$core$Maybe$withDefault,
			$elm$core$Maybe$Nothing,
			A2($elm$core$Array$get, index, g));
		if (!pChar.$) {
			var pc = pChar.a;
			return pc;
		} else {
...
var $author$project$Main$toPasswordString = function (model) {
	var p = model.m;
	var g = model.n;
	return $elm$core$String$fromList(
		A2(
			$elm$core$List$map,
			A2($author$project$Main$getChar, p, g),
			A2($elm$core$List$range, 0, 7)));
};
var $elm$html$Html$tr = _VirtualDom_node('tr');
var $elm$html$Html$Attributes$type_ = $elm$html$Html$Attributes$stringProperty('type');
var $author$project$Main$view = function (model) {
	var pwd = $author$project$Main$toPasswordString(model);
	var pwdElement = A2(
		$elm$html$Html$span,
		_List_Nil,
		_List_fromArray(
			[
				$elm$html$Html$text(pwd)
			]));
	var dbgStr = $elm$core$String$fromInt(model.f);
	return A2(
		$elm$html$Html$table,
		_List_fromArray(
			[
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text('Day 5: How About a Nice Game of Chess?')
									]))
							]))
					])),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$href('https://adventofcode.com/2016/day/5')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text('https://adventofcode.com/2016/day/5')
									]))
							]))
					])),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										model.r ? $elm$html$Html$text('Hack door') : $elm$html$Html$text('Pause')
									]))
							]))
					])),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(' Improved mechanism')
									]))
							]))
					])),
				A2(
