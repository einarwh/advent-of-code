...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {_: fragment, ab: host, P: path, ag: port_, aj: protocol, ak: query};
	});
		...
		while (true) {
			...
			if (stepsLeft > 0) {
				...
				var $temp$acc = A2($elm$core$List$cons, nextPos, acc),
					$temp$dir = dir,
					$temp$stepsLeft = stepsLeft - 1,
					$temp$_v0 = nextPos;
				acc = $temp$acc;
				dir = $temp$dir;
				stepsLeft = $temp$stepsLeft;
				_v0 = $temp$_v0;
				continue moveForwardLoop;
			} else {
				return acc;
			}
		}
	...
	function (inst, _v0) {
		...
		if (!inst.$) {
			...
			var nextPathSegment = A2(
				$elm$core$List$map,
				function (p) {
					return _Utils_Tuple2(nextDir, p);
				},
				nextPositions);
			var nextPos = A2(
				$elm$core$Maybe$withDefault,
				pos,
				$elm$core$List$head(nextPositions));
			return _Utils_Tuple3(
				nextDir,
				nextPos,
...
var $author$project$Main$updateStep = function (model) {
	...
	if (!_v0.b) {
		...
		var _v2 = function () {
			var _v3 = model.x;
			if (!_v3.$) {
				return _Utils_Tuple2(model.x, model.v);
			} else {
				return A2($elm$core$Set$member, pos, model.v) ? _Utils_Tuple2(
					$elm$core$Maybe$Just(pos),
					model.v) : _Utils_Tuple2(
					$elm$core$Maybe$Nothing,
					A2($elm$core$Set$insert, pos, model.v));
			}
		}();
		var twice = _v2.a;
		var seen = _v2.b;
...
var $author$project$Main$toPolyline = function (path) {
	...
	return A2($elm$svg$Svg$polyline, attrs, _List_Nil);
};
...
var $author$project$Main$toSvg = function (model) {
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$core$String$fromInt(xt)),
							$elm$svg$Svg$Attributes$cy(
							$elm$core$String$fromInt(yt)),
	...
	var elements = A2(
		...
		_List_fromArray(
			[polyline]),
		maybeTwiceElement);
	var botRight = _Utils_Tuple2(170, 160);
	return A2(
		$elm$svg$Svg$svg,
		_List_fromArray(
			[
				$elm$svg$Svg$Attributes$viewBox('-170 -170 340 340'),
				$elm$svg$Svg$Attributes$width('340'),
					...
					[
						...
						_List_fromArray(
							[
								...
								A2($elm$html$Html$Attributes$style, 'font-size', '4px'),
								A2($elm$html$Html$Attributes$style, 'padding', '10px')
							]),
						_List_fromArray(
							[
