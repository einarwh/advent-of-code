...
parseSpin : String -> Maybe Move
parseSpin s =
  String.dropLeft 1 s |> String.toInt |> Maybe.map Spin

parseExchange : String -> Maybe Move
parseExchange s =
  if String.startsWith "x" s then
    case s |> String.dropLeft 1 |> String.split "/" of
      [a, b] ->
        case (String.toInt a, String.toInt b) of
          (Just pos1, Just pos2) -> Just (Exchange (pos1, pos2))
          _ -> Nothing
      _ -> Nothing
  else
    Nothing

parsePartner : String -> Maybe Move
parsePartner s =
  if String.startsWith "p" s then
    case s |> String.dropLeft 1 |> String.split "/" of
      [a, b] ->
        case (String.toInt a, String.toInt b) of
          (Nothing, Nothing) -> Just (Partner (a, b))
          _ -> Nothing
      _ -> Nothing
  else
    Nothing
...
parseMove s =
  s
  |> parseSpin
  |> orElseWith (\_ -> parseExchange s)
  |> orElseWith (\_ -> parsePartner s)
...
initModel wholeDance =
  let
    moves = input |> String.split "," |> List.filterMap parseMove
  in
    { order = initialOrder
    , prevOrder = ""
    , dance = moves
...
spin n s =
  let
    right = String.right n s
    left = String.dropRight n s
  in
    right ++ left
...
exchange ix1 ix2 s =
  let
    p1 = String.slice ix1 (ix1 + 1) s
    p2 = String.slice ix2 (ix2 + 1) s
  in
    partner p1 p2 s

partner : String -> String -> String -> String
partner p1 p2 s =
  s |> String.replace p1 "_" |> String.replace p2 p1 |> String.replace "_" p2

makeMove : Move -> String -> String
makeMove move order =
  case move of
    Spin n -> spin n order
    Exchange (ix1, ix2) -> exchange ix1 ix2 order
    Partner (p1, p2) -> partner p1 p2 order

makeDance : List Move -> String -> String
makeDance moves order =
  List.foldl makeMove order moves 

updateStepDance : Model -> Model
updateStepDance model =
  if model.seen |> List.member model.order then 
    let 
      len = List.length model.seen 
      ix = len - (1000000000 |> modBy len) - 1
      order = model.seen |> List.drop ix |> List.head |> Maybe.withDefault model.order 
      prevOrder = model.seen |> List.drop (ix - 1) |> List.head |> Maybe.withDefault model.order 
    in 
      { model | order = order, prevOrder = prevOrder, dances = 1000000000, debug = "LOOP" }
  else 
    let
      seen = model.order :: model.seen 
      prevOrder = model.order
      nextOrder = prevOrder |> makeDance model.dance
    in
      { model | order = nextOrder, prevOrder = prevOrder, seen = seen, dances = model.dances + 1  }

updateStepMove : Model -> Model
updateStepMove model =
  case model.remainingMoves of
    ...
    move :: rest ->
      let
        prevOrder = model.order
        nextOrder = prevOrder |> makeMove move
      in
        { model | order = nextOrder, prevOrder = prevOrder, lastMove = Just move, remainingMoves = rest }

updateStep : Model -> Model
updateStep model =
  if model.wholeDance then
    updateStepDance model
  else
    updateStepMove model
...
updateTogglePlay model =
  if model.finished then
    let
      m = initModel model.wholeDance
    in
      {m | paused = False }
  else
    { model | paused = not model.paused }

updateToggleWholeDance : Model -> Model
updateToggleWholeDance model =
  let
    wholeDance = not model.wholeDance
  in
    initModel wholeDance
...
update msg model =
  case msg of
    ...
    ToggleWholeDance ->
      (updateToggleWholeDance model, Cmd.none)
    Faster ->
...
createLetterElements boxSize y order =
  order |> String.toList |> List.indexedMap (\x ch -> toLetterElement boxSize ch (x, y))

createBoxes : Float -> Int -> List (Svg Msg)
createBoxes boxSize y =
  List.range 0 15  |> List.map (\x -> toBoxElement boxSize (x, y))

toConnectionElement : Float -> (Int, Int) -> Svg Msg
toConnectionElement boxSize (xIntSrc, xIntTgt) =
  let
    x1s = (0.5 + toFloat xIntSrc) * boxSize |> String.fromFloat
    x2s = (0.5 + toFloat xIntTgt) * boxSize |> String.fromFloat
    y1s = String.fromFloat <| 1 * boxSize
    y2s = String.fromFloat <| 5 * boxSize
  in
    Svg.line
      [ x1 x1s
      , y1 y1s
...
indexOf p order =
  order |> String.indexes p |> List.head |> Maybe.withDefault 0

findConnections : String -> String -> List (Int, Int)
findConnections prevOrder order =
  if String.isEmpty prevOrder then []
  else
    let
      programList = initialOrder |> String.toList |> List.map String.fromChar
    in
      programList
      |> List.map (\p -> (indexOf p prevOrder, indexOf p order))
...
toSvg model =
  let
    boxSize = toFloat 24
    connections = findConnections model.prevOrder model.order
    connectionElements = connections |> List.map (toConnexionElement boxSize)
    letterElements =
      if String.isEmpty model.prevOrder then
        createLetterElements boxSize 1 model.order
      else
        let
          topRowLetterElements = createLetterElements boxSize 1 model.prevOrder
          botRowLetterElements = createLetterElements boxSize 6 model.order
        in
          List.append topRowLetterElements botRowLetterElements
    boxElements =
      if String.isEmpty model.prevOrder then
        createBoxes boxSize 0
      else
        List.append (createBoxes boxSize 0) (createBoxes boxSize 5)
    -- defs = Svg.defs [] [ createMarker "arrowhead" ]
    elements = List.concat [ boxElements, connectionElements, letterElements ]
  in
    svg
      [ viewBox "-5 -15 395 170"
      , width "400"
      , height "175"
...
unparse move =
  case move of
    Spin n -> "s" ++ String.fromInt n
    Exchange (ix1, ix2) -> "x" ++ String.fromInt ix1 ++ "/" ++ String.fromInt ix2
    Partner (p1, p2) -> "x" ++ p1 ++ "/" ++ p2
...
view model =
  let
    ...
    moveStr =
      if model.wholeDance then 
        if model.dances > 0 then String.fromInt model.dances else ""
      else 
        case model.lastMove of
          Just move -> unparse move
          Nothing -> ""
    dbgStr = ""
  in
    Html.table
      ...
      [ Html.tr
          ...
          [ Html.td
              ...
              , Html.div [] [Html.text "Day 16: Permutation Promenade" ] ] ]
      , Html.tr
          []
          [ Html.td
              [ Html.Attributes.align "center"
              ...
              [ Html.a
                [ Html.Attributes.href "https://adventofcode.com/2017/day/16" ]
                [ Html.text "https://adventofcode.com/2017/day/16" ]
            ] ]
      , Html.tr
          []
          [ Html.td
              ...
              , Html.button
                ...
                [ if model.paused then Html.text "Dance" else Html.text "Pause" ]
              , Html.button
      ...
      , Html.tr
          ...
          [ Html.td
              [ Html.Attributes.align "center" ]
              [ Html.input
                [ Html.Attributes.type_ "checkbox", onClick ToggleWholeDance, Html.Attributes.checked model.wholeDance ]
                []
              , Html.label [] [ Html.text " Whole dance" ]
            ] ]
      , Html.tr
          []
          [ Html.td
